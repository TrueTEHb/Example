...
public class AdminController {
    ...
    public ResponseEntity<?> dataPage() {
        ...
        model.addAttribute("people", restService.getAllUsers());
    ...
    public ResponseEntity<?> deleteUser(@PathVariable("id") Long id) {
        ...
        User existUser = restService.getUser(id);
    ...
    public ResponseEntity<?> updateUser(@RequestBody Map<String, Object> user1, @PathVariable Long id) {
        User forRole = restService.getUser(id);
        ...
        if (pass.isEmpty() || pass.equals("")) {
            pass = restService.getUser(id).getPassword();
        }
        ...
        if (loginedUser.getId() == id) { // свою роль нельзя изменить
            User finalUser = restService.updateUser(loginedUser, null);
            model.addAttribute("user", finalUser);
            return ResponseEntity.ok(model);
        }
        User finalUser = restService.updateUser(editUser, role);
        model.addAttribute("user", finalUser);
        return ResponseEntity.ok(model);
    }
    ...
    public ResponseEntity<?> addNewUser(@RequestBody Map<String, String> param) {
        ...
        User user = restService.getUserByEmail(email);
        if (user != null) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        } else {
